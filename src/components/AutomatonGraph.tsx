...
interface AutomatonGraphProps {
  ...
  highlightedTransitions?: { from: string; to: string; label: string; color?: string }[];
  width?: number;
  height?: number;
  labelPosition?: 'inside' | 'top' | 'bottom';
  stateColor?: string | ((id: string) => string);
}
...
const AutomatonGraph: React.FC<AutomatonGraphProps> = ({
  ...
  useEffect(() => {
    ...
    const links: Link[] = Object.entries(groupedTransitions).map(([key, labels]) => {
      ...
      const highlightMatch = highlightedTransitions.find(ht => 
        ht.from === from && ht.to === to
      );
      const isHighlighted = !!highlightMatch;
      const highlightColor = highlightMatch?.color;
      ...
      if (!link) {
        link = { source: from, target: to, label: "", isHighlighted: false, highlightColor: undefined, hasReverse: false } as Link;
      }
    ...
    if (!structureChanged && simulationRef.current) {
      ...
      svg.selectAll<SVGPathElement, Link>(".link")
        ...
        .transition()
        .duration(200)
        .attr("stroke", d => d.isHighlighted ? (d.highlightColor || "#ef4444") : "#999")
        .attr("stroke-width", d => d.isHighlighted ? 4 : 2)
        .attr("marker-end", d => {
          if (!d.isHighlighted) return "url(#arrowhead)";
          if (d.highlightColor === "#fbbc33") return "url(#arrowhead-highlight-secondary)";
      ...
      svg.selectAll<SVGTextElement, Link>(".link-labels text")
        ...
        .transition()
        .duration(200)
        .attr("fill", d => d.isHighlighted ? (d.highlightColor || "#ef4444") : "#333")
      ...
      svg.selectAll<SVGGElement, Node>(".node")
        ...
        .each(function(d) {
          ...
          const defaultFill = d.isStart ? "#e0f2fe" : "#fff";
          const customFill = typeof stateColor === 'function' ? stateColor(d.id) : stateColor;
          const fill = d.isHighlighted ? "#fee2e2" : (customFill || defaultFill);
    ...
    defs.append("marker")
      .attr("id", "arrowhead-highlight-secondary")
      .attr("viewBox", "0 -5 10 10")
      .attr("refX", 8)
      .attr("refY", 0)
      .attr("markerUnits", "userSpaceOnUse")
    ...
    node.append("circle")
      ...
      .attr("fill", d => {
        const defaultFill = d.isStart ? "#e0f2fe" : "#fff";
        const customFill = typeof stateColor === 'function' ? stateColor(d.id) : stateColor;
        return d.isHighlighted ? "#fee2e2" : (customFill || defaultFill);
      })
    ...
    node.append("text")
      ...
      .attr("fill", "#141414")
      .attr("stroke", "none")
      .text(d => d.id.toString().replace(/^[12]_/, ''));
